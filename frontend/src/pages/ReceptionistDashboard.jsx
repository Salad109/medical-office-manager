    ...
    <div className="d-grid gap-4">
      ...
      <div className="glassy-card shadow-sm p-4">
        <div className="d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-3 mb-4">
          ...
          <button type="button" className="btn btn-outline-primary" onClick={loadPatients} disabled={loadingPatients}>
            Refresh
          </button>
        </div>
        <form className="row g-3 mb-4" onSubmit={handlePatientSubmit}>
          ...
          <div className="col-12 d-flex justify-content-between gap-2">
            <button type="button" className="btn btn-outline-secondary" onClick={() => setPatientForm(createPatientFormState())}>
              Clear
            </button>
                ...
                {patients.map((patient) => (
                  <tr key={patient.id}>
                    ...
                    <td className="text-end d-flex gap-2 justify-content-end">
                      ...
                      <button type="button" className="btn btn-outline-danger btn-sm" onClick={() => handlePatientDelete(patient.id)}>
                        Delete
                      </button>
                    </td>
                  </tr>
      ...
      <div className="glassy-card shadow-sm p-4">
        <div className="d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-3 mb-4">
          ...
          <button type="button" className="btn btn-outline-primary" onClick={loadAppointments} disabled={loadingAppointments}>
            Refresh
          </button>
        </div>
        <form className="row g-3 mb-4" onSubmit={handleAppointmentSubmit}>
          ...
          <div className="col-12 d-flex justify-content-between gap-2">
            <button type="button" className="btn btn-outline-secondary" onClick={() => setAppointmentForm(createAppointmentFormState())}>
              Clear
            </button>
            <button type="submit" className="btn btn-primary">
              ...
              <tbody>
                {appointments.map((appointment) => (
                  <tr key={appointment.id}>
                    <td>
                      <div className="fw-semibold">{appointment.patientName || appointment.patientId}</div>
                      <div className="text-muted small">#{appointment.patientId}</div>
                    </td>
                    <td>
                      <div className="fw-semibold">{appointment.doctorName || appointment.doctorId}</div>
                    ...
                    <td className="text-end d-flex gap-2 justify-content-end">
                      ...
                      <button type="button" className="btn btn-outline-danger btn-sm" onClick={() => handleAppointmentDelete(appointment.id)}>
                        Delete
                      </button>
                    </td>
                  </tr>
